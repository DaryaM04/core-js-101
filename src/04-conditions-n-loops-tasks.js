...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const oneElem = a < b ? a : b;
  const endElem = b > a ? b : a;
  return `${open}${oneElem}, ${endElem}${close}`;
}
...
function reverseString(str) {
  const one = str.split(' ').reverse();
  const result = one.map((elem) => elem.split('').reverse().join(''));
  return [result.join(' ')];
}
...
function reverseInteger(num) {
  return Array.from(String(num).split('')).reverse().join('');
}
